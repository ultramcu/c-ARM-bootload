 ...
 *
 * STM32F1 hook implementations: flash erase / program / read on the
 ...
 * ~30 ms per 1 KiB page on STM32F103, so a full 47-page application
 * erase takes well over a second -- main.c sizes the IWDG to give
 * margin. On the success path flash is left unlocked so that the
 * subsequent stream of flash_write() calls doesn't pay an unlock
 * round-trip per word; flash_write() does not lock again either. */
